...
UBOOT_PATH=u-boot
KERNEL_PATH=device/rockchip/rk3399
rm -rf $IMAGE_PATH
mkdir -p $IMAGE_PATH
...
if [[ $TARGET_PRODUCT = "rk3399_firefly_edp_box" ]]; then
	KERNEL_IMG=kernel_edp_box.img
fi
	...
	then
        if [ "$PRODUCT_SYSTEM_VERITY" = "true" ]; then
                ...
                while [ "$ok" = "0" ]; do
                    ...
                    make_ext4fs -l $system_size $MAKE_EXT4FS_ARGS  2>&1 &&
                    #tune2fs -c -1 -i 0 $IMAGE_PATH/system.img >/dev/null 2>&1 &&
                    tune2fs -c -1 -i 0 $IMAGE_PATH/system.img  2>&1 &&
...
if [ -f $KERNEL_PATH/$KERNEL_IMG ]
then
        echo -n "create kernel.img..."
        cp -a $KERNEL_PATH/$KERNEL_IMG $IMAGE_PATH/kernel.img
        echo "done."
else
	echo "$KERNEL_PATH/$KERNEL_IMG not fount!"
fi
